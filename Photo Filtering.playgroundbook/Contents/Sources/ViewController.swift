...
public class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    private var enabledTouchQuestionButton: Bool = true
    ...
    @objc func didTouchUndo() {
        undoButton.animateTouchedButton()
        guard currentImageIndex > 0 else { return }
        currentImageIndex -= 1
    }
    
    @objc func didTouchRedo() {
        redoButton.animateTouchedButton()
        guard currentImageIndex < imageStack.count-1 else { return }
        currentImageIndex += 1
    }
    
    @objc func didTouchQuestionButton() {
        if self.enabledTouchQuestionButton == false || descriptionView.isHiddingView {
            return
        }
        
        self.enabledTouchQuestionButton = false
        questionButton.animateTouchedButton()
        ...
        descriptionView.showView(toFrame: toFrame) { [weak self] in
            self?.enabledTouchQuestionButton = true
        }
    }
